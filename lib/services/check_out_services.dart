...
class _CheckoutPageState extends State<CheckoutPage> {
  ...
  // UPDATED: Now uses the same BuyNowBottomSheet approach
  void _proceedToBuyingSheet() {
    // For cart checkout, we'll use the first item's details and pass cart context
    ...
    showModalBottomSheet(
      ...
      builder: (context) => CartBuyNowBottomSheet(
...
// NEW: Cart-specific version of BuyNowBottomSheet
class CartBuyNowBottomSheet extends StatefulWidget {
  final List<CartItem> cartItems;
  final Map<String, double> cartTotals;
  final OrderService orderService;

  const CartBuyNowBottomSheet({
    Key? key,
    required this.cartItems,
    required this.cartTotals,
  ...
  @override
  State<CartBuyNowBottomSheet> createState() => _CartBuyNowBottomSheetState();
}

class _CartBuyNowBottomSheetState extends State<CartBuyNowBottomSheet> {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildSummaryRow('Subtotal', 'Rs${widget.cartTotals['subtotal']?.toStringAsFixed(2)}'),
          _buildSummaryRow('Shipping', 'Rs${widget.cartTotals['shipping']?.toStringAsFixed(2)}'),
          _buildSummaryRow('Tax (8%)', 'Rs${widget.cartTotals['tax']?.toStringAsFixed(2)}'),
  ...
  Future<void> _placeOrder() async {
    ...
    try {
      ...
      // Create separate orders for each cart item (maintaining consistency with existing structure)
      ...
      for (CartItem item in widget.cartItems) {
        final orderData = {
          ...
          'cartOrderId': DateTime.now().millisecondsSinceEpoch.toString(),
